...
class GoogleMapsApiClient:
    ...
    def create_place(self, lat: float, lng: float, name: str, address: str) -> str:
        ...
        # Проверка через assert (заменяет if/else)
        ...
        place_id = resp_json.get("place_id")
    ...
    def get_place_details(self, place_id: str) -> Dict[str, Any]:
        ...
        data = response.json()
        # Если ответ содержит "msg" – значит место не существует
...
class TestGoogleMapsPostGetFile:
    ...
    def __init__(self) -> None:
        ...
        self.filename = "place_ids.txt"

    def save_place_ids(self, ids: List[str]) -> None:
    ...
    def run_test(self) -> None:
        ...
        read_ids = self.read_place_ids()

        # Проверяем, что прочитали столько же, сколько сохранили
        assert len(read_ids) == len(place_ids), \
            f"Количество прочитанных id ({len(read_ids)}) не совпадает с сохранёнными ({len(place_ids)})"

        for pid in read_ids:
            # GET запрос сам проверяет существование через assert
